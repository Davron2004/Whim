...
### 4.10 The git-cleanup lane (BUILT + validated live 2026-07-06; runbook: `.claude/commands/git-cleanup.md`) — freed path, gated outcome

A second grant category, on the **ref plane** rather than the file plane, for one recurring task: squashing careless commit runs on `main` into a readable history (first run: 159 → 23 commits). It inverts the fixer model deliberately: fixers get a *narrow allowlist* because a code diff can't be verified by one cheap check, so we constrain the path; the cleaner gets a *narrow denylist* and near-total process freedom because its outcome CAN be verified by one cheap check — **tip-tree identity**. If `cleanup/main-squashed^{tree}` equals the tree pinned at grant time, no content change of any kind survived the rewrite; the only thing the agent can shape is commit structure and messages, which is the deliverable itself. Verification strength buys process freedom.

**Why a worktree is NOT the boundary here:** a linked worktree shares the entire `.git` (refs, objects, reflog, config) with the main checkout — worktree isolation isolates *trees*, not *history*. So the lane's fence sits at **shared state**, not at the worktree path. Tier-1 denies in `bash-policy.sh` hold for every caller *including the lane*: network git, forced moves/deletes/renames of `main`/`dev/v1` (incl. `checkout -B`/`switch -C`), `tag -f`/`-d`, `update-ref`/`symbolic-ref`, `reflog`/`gc` (the reflog + backup ref are the undo button — the agent must not destroy recovery data), and `git config` (a linked worktree's `git config` writes the SHARED `.git/config`; `core.fsmonitor` there is code execution in the main tree). Tier-2 (`git rebase`, history rewrite generally) is denied everywhere EXCEPT inside the active lane.

**Lane activation = three unforgeable conditions** (`cleanup_lane()` in `bash-policy.sh`): the human-minted grant file `.claude/fixloop/grants/git-cleanup` exists (agent-unwritable — same trust anchor as Class-1 grants), the command runs in the HARDCODED worktree `.claude/worktrees/main-squashed` — via cwd **or via a verbatim `git -C <lane-worktree-root> …` prefix**, root derived from the path, no cwd trust — and the agent owns it via `owners_claim` (same binding as fixers). The `-C` form is load-bearing, learned live: **the harness re-pins a subagent's cwd to the repo root on every Bash call (`cd` does not persist)**, so location-keyed policy must accept a path argument the hook can verify, not a working directory the agent must occupy. Inside the lane, the full simple-command git vocabulary is allowed (minus tier-1 and `git worktree`); with no grant file the lane doesn't exist and policy is byte-identical to before for every caller. Compound commands keep falling through — no chained payload is ever auto-allowed.

**The grant file pins the reference data before the agent runs** (pinned-BASE philosophy): `main_sha=<sha>` and `main_tree=<tree>`. The orchestrator also creates `backup/pre-cleanup` at pinned main and the lane worktree with branch `cleanup/main-squashed` before dispatch (the `git worktree add` needs the OS sandbox off — checkout materializes `.claude/` copies the sandbox won't write).

**The rebuild is INDEX-ONLY** (`.claude/agents/git-cleaner.md`): `git reset --soft <base>` once, then per group boundary `git read-tree <B>` + `git commit` — snapshots, not replayed diffs, so it cannot conflict, linearizes PR merges, and never touches working files (`reset --hard`/`checkout --`/`read-tree -u` would die on the sandbox's `.claude/`-copy deny). The final boundary is main's tip itself, making tree identity true by construction.

**The outcome gate is `scripts/git-cleanup-check.sh`** (protected like the gate it joins — PROTECTED regex + protect-harness, repo-root and in-worktree): (1) branch tip tree == pinned tree, (2) `main` still == pinned SHA (retroactively catches anything that slipped the ref fence), (3) backup ref intact. Wired into `gate-on-subagent-stop.sh` for `agent_type = git-cleaner` — the cleaner cannot finish until it passes (self-reported verdicts are never trusted; same reasoning as sandbox finding F4). On pass it PRINTS the human merge (`git checkout main && git reset --hard cleanup/main-squashed` — a pure ref move relative to *committed* content — then `git push --force-with-lease`) and the teardown; it executes nothing. **Sharp edge, learned live: that `reset --hard` also discards any UNCOMMITTED tracked changes in the main tree** — the runbook's clean-tree precondition exists precisely because the first run wiped the lane's own then-uncommitted hook edits.

**Lifecycle (one-shot, re-runnable):** human/main-thread mints grant + backup ref + worktree (each through the `ask` prompt) → dispatch `git-cleaner` → stop-gate passes only on tree identity → human skims the squash map, runs the printed ref-move + force-push → delete grant + owner marker + worktree + branch. Grants are minted per run, never standing. Concurrency rule: never mint this grant while `.claude/fixloop/owners/` shows an active fixer run — history rewrite under a live pinned-BASE audit is the one interaction that could confuse it.
...
### 6.10 Critic-driven hook hardening — DONE (2026-07-02, main-thread-authored + human-ratified live)

The 2026-07-02 critic sweep of the harness range surfaced five findings; all five fixed the same day (hook edits proposed by the main thread, each Edit human-approved at the prompt — the Class-2 path working as designed). Piped-JSON hook suite: 24/24.

- **`invariants/` main-tree gap closed**: added to protect-harness's main-tree block-list and bash-policy's PROTECTED write regex — it was only protected *inside worktrees* (the 8b503ae rebuild hardened the worktree case and left the main-tree case as-was, i.e. hook-unenforced for the feature-chain implementers who actually run there).
- **Subshell write-verb bypass closed**: `BND` gains `(` — `(cp x package.json)` slipped both the boundary anchor and the compound-indicator list. Same one-character class of bug as the fix it hid inside (aeccc05).
- **Fixloop guard narrowed to invocation shapes**: the match-anywhere substring denied *reads* (`git diff`/`cat`) and even prose quoting the path — the critic's own report write tripped it, twice. Now only direct-exec-at-segment-start or interpreter invocation matches; env-prefixed invocation (`VAR=x <script>`) still slips it — accepted, the guard is anti-accident (backstops: pinned-BASE diff + review).
- **Agent↔worktree ownership binding (closes the sibling-worktree reach)**: `owners_claim` in bash-policy binds a worktree to the FIRST subagent that uses it (atomic noclobber marker at `.claude/fixloop/owners/<wt-id>` — agent-unwritable, same trust model as grants) and refuses other agents' `cd`/scoped-git. The cwd-scoped git vocabulary previously checked only "under `.claude/worktrees/`", not "under MINE", so a worker could mutate a sibling's tree. The orchestrator (no `agent_id`) is never checked; `finish` prints the marker `rm`, and a parked worktree handed to a NEW agent needs its marker removed first.
- **`useRef` decision recorded**: decision #44 + `docs/sdk-gap.md` §1 annotated (the SDK-surface expansion had shipped with no decision-log entry).
