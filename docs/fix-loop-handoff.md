...
1. **`sandbox.enabled: true` — STILL PENDING THE HUMAN** (the proportionate fix — right layer; closes Threat C). Tuning when enabled: deny credential reads (`~/.ssh`, `~/.aws`, `.env`); network is prompt-gated (autonomous subagents can't approve → effectively no egress); writes confined to workdir+temp. The fix-loop itself is sandbox-compatible (all offline/in-repo). Recommend enabling **before any unattended/scaled run**. (The human must do this — settings self-mod is blocked for the agent.)
   ...
   - **Chromium does NOT run under macOS Seatbelt** (confirmed 2026-06-29): headless Chromium dies at launch with `FATAL mach_port_rendezvous_mac.cc … bootstrap_check_in … Permission denied (1100)` — Seatbelt blocks the Mach-port IPC Chromium's multi-process model needs (it's already `--no-sandbox` for its own; this is the *OS* sandbox). No sandbox flag fixes it on macOS (`enableWeakerNestedSandbox` is Linux-only; `allowAppleEvents` is the wrong issue and degrades isolation). **Resolution: `excludedCommands`.** This is threat-model-clean: the untrusted fixer runs only the FAST gate (no Chromium, stays sandboxed); the Chromium steps live in `gate-full`, run by the trusted orchestrator on an integrity-checked fresh checkout, and the adversarial bundles they execute are contained by the *runtime under test* (iframe+CSP), not Seatbelt. Residual: `gate-full` then runs unsandboxed (no `node_modules` denyWrite for that once-per-fix, post-review step). The repo's three Chromium scripts + the two wrappers:
     ...
     "excludedCommands": [
       ...
       "./scripts/gate-full.sh", "scripts/fixloop.sh gatefull"
     ]
     ```
     Verified 2026-06-29: with these excluded, all three Chromium suites pass while `~/`+`node_modules` writes stay blocked and `build` stays sandboxed. (`excludedCommands` matching semantics are undocumented — the nested wrapper case gets its real test at the first `gate-full`.) Situational extras only if you drive them through the agent: `npm run invariants:spike2` (vendored Playwright), the Android build (`react-native run-android` — needs network + `~/.gradle` + Android SDK), `npm start`/`server:dev` (Metro port), `ssh` (laptop).
