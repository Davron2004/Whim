...
3. **Real-finding shakeout — DONE 2026-06-29 (on F1, `server/src/sse.ts` SSE keepalive leak).** Loop logic validated end-to-end: plan → fix (`53da72b`, branch `worktree-agent-a304b942cfcba5d0b`) → red-check (RED, non-vacuous) → integrity (clean) → reviewer (ACCEPT). Two harness defects surfaced + fixed: (a) sandbox blocks `git worktree add` → exclude whole `scripts/fixloop.sh` (§6.1); (b) Metro can't bundle from an in-repo worktree → `gatefull` rewritten to run in the MAIN tree from the committed tip (§4.7). **F1 is verified-good but NOT yet merged** — held until (a)+(b) land, then a clean `gatefull` re-run, then merge (decision 2026-06-29). The new fixloop.sh `gatefull` is uncommitted + untested end-to-end (the dirty-tree guard fires correctly; needs commit + settings update + restart to run for real).
