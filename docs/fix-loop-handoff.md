...
> ⚠️ **Two things the human has NOT done yet** (as of this handoff): (1) reviewed/committed the uncommitted driver files (§3); (2) read the security findings in §6. Don't assume either is settled.
...
**UNCOMMITTED in the working tree — the orchestrator driver, pending the human's review + commit:**
```
?? scripts/fixloop.sh                     # NEW — deterministic toolkit
?? .claude/agents/fix-worker.md           # NEW — the fixer agent
?? .claude/commands/fix-loop.md           # NEW — the /fix-loop runbook
 M .claude/hooks/bash-policy.sh           # + fixloop.sh protections (PROTECTED + subagent-run deny)
 M .claude/hooks/protect-harness.sh       # + fixloop.sh edit-block
 M docs/parallel-fix-loop.md              # + §5.1 (driver implementation note)
```
Suggested commit message:
`harness: fix-loop orchestrator driver (fixloop.sh toolkit + fix-worker agent + /fix-loop runbook), protect fixloop.sh`

> 🚨 **Until these are committed, `./scripts/gate.sh` will REFUSE to run** — its pinned-BASE tripwire watches `.claude/hooks`, which is currently dirty. This is by design (4.7 fix). Commit first, then the gate runs.
> Note: `scripts/fixloop.sh` is created non-exec by the Write tool — confirm it has the exec bit (`chmod +x`) and commit the mode, like we did for `gate-full.sh`.
...
## 6. OPEN: the security question the human raised (unread by them yet)
...
**Decisions pending the human (do NOT act without their say):**
1. **Enable `sandbox.enabled: true`** (the proportionate fix — right layer). Tuning when enabled: deny credential reads (`~/.ssh`, `~/.aws`, `.env`); network is prompt-gated (autonomous subagents can't approve → effectively no egress); writes confined to workdir+temp. **Caveat:** writes outside the workdir break — notably the **memory dir** (`~/.claude/projects/.../memory/`) — so allow-list those few paths. The fix-loop itself is sandbox-compatible (all offline/in-repo). Recommend enabling **before any unattended/scaled run**. (The human must do this — it's session-wide, set at start, and settings self-mod is blocked for the agent.)
2. **Close two Threat-B gaps regardless of the sandbox:** (a) add `build/` to the PROTECTED set (it's subagent-writable *and* executed via auto-allowed `npm run build`); (b) make the orchestrator run `gate-full` in a **fresh checkout of the branch**, not the agent's (possibly-poisoned) worktree.

## 7. Next steps (in order)

1. **Human:** review + commit the §3 driver files (and chmod+commit `fixloop.sh`'s exec bit). Until then the gate won't run.
2. **Human:** read §6, decide on the sandbox + the two B-gaps.
