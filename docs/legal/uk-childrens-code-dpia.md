...
| 15. Online tools | In-app report sheet. Settings switches for AI features and error details. The phone ID is shown in Settings, with "Make a new ID" (B4). Rights requests go to `support@whim.anycognition.ca`. | Rights requests need an email. An in-app request tool would be easier for a teen; not planned. |
