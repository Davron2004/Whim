...
|---|---|---|
| Fast gate | `scripts/gate.sh` | build → typecheck → lint → the five Node suites → scaffolding tripwires. Runs on every inner-loop attempt, in the agent's worktree. Refuses to run if any protected config differs from `GATE_BASE` (pinned-BASE tamper tripwire). |
