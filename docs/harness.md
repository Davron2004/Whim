...
- **The per-run staging branch (`integration/<run-id>`) is the single-writer integration
  ...
  unset ⇒ legacy direct-to-`main` behavior).
...
### 4.1 Three policy behaviours that mislead if you meet them cold

All three are **fail-closed and correct**. They are documented here because each one's *denial
message points somewhere other than its cause*, and an operator who misreads it draws a wrong
conclusion about the harness — which has already happened once, producing a finding
(`harden-closure-lane` §F2) that was wrong about which component was broken.

**(a) The two relaxed remote forms are relaxed only as BARE commands.** `bash-policy.sh` drops any
command containing `&`, `;`, `|`, a backtick, `$(`, `>`, `<` or a newline out of the relaxation and
back to the tier-1 deny. A pipe as innocent as `| tail -3` is enough. The denial you then get names
the **network** rule — "git network/shared-ref/history op is human-approved only" — with no hint
that compounding was the trigger, so it reads as "the relaxation is gone". It is not. Issue those
two forms one per call, on their own.

**(b) The matcher judges command TEXT, including content the command merely carries.** A heredoc
body, a `--body "…"` PR description, or a `grep` *search pattern* that happens to contain protected
vocabulary is denied exactly as if it were the operation itself. This is deliberate: exempting
"content" would require the hook to parse quoting well enough to know what is content, and getting
that wrong is a real bypass. Fail-closed is the right trade.

> **The sanctioned workaround is to pass the text through a file, never to reword it.**
> `Write` the body to a path, then `gh pr create --body-file <path>`; `git commit -F <path>` instead
> of `-m`; the `Edit`/`Write` tools instead of a shell heredoc. Rewording prose until the guard
> stops firing is the one response to avoid — it trains the reflex of dodging the matcher, and it
> silently degrades the message you were trying to write.

**(c) `gh` fails under the sandbox at the TRUST STORE, not at the network.** See the sandbox row
above for the policy story; the mechanism is §11.

### 4.2 Why closure's ancestor check is local and stays local

Step 12f uses `git merge-base --is-ancestor main integration/<run-id>` rather than the hosting
provider's mergeability report. This is deliberate and **measured**, not an oversight to be
"simplified" later: the local check is not in the tier-1 deny list at any position and returns
`rc=0` from the main thread **inside the sandbox**, whereas any `gh` query needs an explicit
unsandboxed override (§4.1c, §11). Swapping a sandbox-runnable gate for one that cannot run in the
...
and every `git push` is either denied outright (subagents always; anything naming `main`
