...
Cross-agent mirror: `AGENTS.md` is a **symlink** to this file, and `.codex/hooks/*.sh` are symlinks to `.claude/hooks/*.sh` — never write them separately. `.codex/agents/*.toml` are **generated** from `.claude/agents/*.md` by `scripts/sync-codex.mjs` (`--write` after editing an agent; `gate-full.sh` runs `--check`). See `docs/harness.md` §10.
