...
**`docs/harness.md` is the single source of truth for the coding harness** — roles, trust model, enforcement map, both loops, and the do-not-re-derive gotchas. The superseded design docs live in `docs/archive/` (kept for spike evidence); diagrams in `docs/coding-harness-diagram.md`. Each run integrates on its own per-run staging branch (`integration/<run-id>`, one active at a time, `FIXLOOP_INTEGRATION_BRANCH`); **`main` is the published branch**, receiving exactly one ratified merge per run at closure.

Two agentic loops share the same worktree-parallel primitives. **`/opsx:apply <change-id>`** is the OpenSpec build loop and routes by schema (`/dispatch` no longer exists — it was folded in): `whim-harness` changes get the chain dispatch loop (runbook: `.claude/commands/opsx/apply.md`) — one **implementer** subagent per context chain from `chains.md`, each in an orchestrator-created worktree branched from the run's staging tip, self-gated, integrity-checked, serially merged onto the staging branch with a post-merge regate, then `gate-full.sh` + a **reviewer** pass; `whim-fixloop` changes route to **`/fix-loop <findings>`** (runbook: `.claude/commands/fix-loop.md`) — N mechanical fixes in parallel under `.claude/worktrees/` (each made by `scripts/worktree.sh create`, which gives it its own `node_modules`: never symlink one), each red-checked, integrity-checked against a **pinned BASE commit recorded at worktree creation** (never HEAD), full-gated from the branch's committed tip checked out into the **main tree** (so untracked or ignored files in the worker's worktree never reach the gate), and serially merged onto the staging branch. The main thread NEVER implements inline. Closure (attended-only) then pushes the staging branch as a draft PR into `main`, iterates SonarCloud findings via a nested `/fix-loop` on that same staging branch until green, runs `/git-cleanup` (target-parameterized onto the staging branch, not `main`) before the single ratified merge. A **critic** (`/critic-run`) files a daily problems report.

The mechanical definition of done is the **gate split**: `scripts/gate.sh` (fast — build + typecheck + lint + the Node suites + scaffolding tripwires, no Metro/Chromium) for every inner-loop attempt, `scripts/gate-full.sh` (full — `gate.sh` plus knip, `guard:metro`, the three Chromium invariant suites, `openspec validate`) once per change/fix before merge. **Disabled since 2026-09-21 (owner decision; kept for a possible return in some form): the protected-file system described in the rest of this paragraph — the `protect-harness.sh` block, Class 1 grants, Class 2 ratification. Agents may edit these files. Still live and separate: the gate's `CONFIG_SET` tripwire (`scripts/gate.sh`) refuses to run while any of them differs from the base commit, so commit such an edit into the base before gating.** Agents cannot edit the gate or its config — `.claude/hooks/protect-harness.sh` hard-blocks subagents (and prompts the main thread) on `scripts/gate*.sh`, `scripts/fixloop.sh`, `scripts/git-cleanup-check.sh`, `scripts/sync-codex.mjs`, `scripts/worktree.sh`, `.claude/**`, `.codex/**`, `package.json`, `package-lock.json`, `tsconfig*.json`, `.eslintrc*`/`eslint.config.*`, `.eslintignore`, `knip.json`, `babel.config.js`, `metro.config.js`, `invariants/`, and `build/*` (the build script runs inside the gate, so a tampered one runs arbitrary code there too). Class 2 is human-ratified: subagents are always denied; an attended Codex root task with `approvals_reviewer = "user"` may apply one exact SHA-256-bound patch through `.codex/hooks/apply-reviewed-protected-patch.sh`, whose prompt, root-transcript binding, immutable Git-private snapshot, one-shot grant, and denial/replay/TOCTOU/rename tests are enforced by the fast gate. Direct protected `apply_patch` remains denied. Information flows through files in the change folder (`research.md` → proposal artifacts → `chains.md` → `handoff/*.md` → `progress.md`), never through context.
...
Unattended/headless `/fix-loop` runs go through `.devcontainer/` (Docker, default-deny egress except the Anthropic API via `init-firewall.sh`) rather than the host OS sandbox — Chromium can't run under macOS Seatbelt. **`sandbox.excludedCommands` in `.claude/settings.json` was measured inert: no entry took effect** (2026-07-26 four-run probe, `openspec/changes/archive/2026-07-27-harden-gate-preconditions/research.md`), so nothing is actually carved out by it. Treat every sandboxed harness command as sandboxed. Two consequences, kept separate because their epistemic status differs: writes under `.claude/**` are denied *architecturally* — Claude Code documents that the sandbox denies writes to its own settings at every scope so a sandboxed command cannot widen its own policy, and that denial is **not** governed by `excludedCommands` and survives `sandbox.filesystem.disabled` (measured here to cover `.claude/hooks`, `.claude/commands`, `.claude/agents`, though the docs name only `settings.json`); and `gh` stayed blocked too — which was recorded as filesystem-independent and unexplained, and is now **resolved** (2026-07-27, `openspec/changes/archive/2026-07-28-harden-closure-lane`): it is not an egress block at all. Sandboxed `gh` fails with `tls: failed to verify certificate: x509: OSStatus -26276` — `errSecInternalComponent`, a macOS **Keychain** error — because `gh` verifies TLS through the system trust store via the Security framework, which the sandbox denies; unsandboxed the identical command exits 0, and `git`'s `osxkeychain` helper fails the same way. So it is a **trust-store access failure presenting as an egress block**, filesystem/IPC-dependent like the `.claude/**` denial rather than independent of it — which is why `excludedCommands` was never the lever and why adding entries there cannot fix it. Measured identically for the main thread and a subagent, so not an agent-identity effect. Practical consequence: every closure `gh` call needs an explicit per-command unsandboxed override. Because this repo versions its control plane *inside* `.claude/`, checking out any branch that changes the harness is sandbox-incompatible **by construction and permanently** — such commands need an attended explicit sandbox override or the devcontainer, and no configuration change can grant it. The override's safety assumption (a human sees the prompt) only holds in an *attended, foreground* session — it silently doesn't fire in background/auto-mode (`docs/archive/parallel-fix-loop.md` §6.7). `scripts/fixloop.sh gatefull` now detects a checkout that half-applied instead of letting the gate mis-report it as tamper (`scripts/test/fixloop-preflight.test.sh`).
...
Node test helpers should delegate to `node:assert` (`ok`, `strictEqual`, `deepStrictEqual`, `rejects`, etc.). The shared `checks/test/harness.ts` helper uses `nodeAssert.ok(condition, message)`: failures still reach the custom runner, and Sonar's S2699 rule can follow the helper to a recognized assertion. Do not import `node:assert` into device/Hermes/WebView or product modules.

Assert observable behavior with conditions that would fail if the implementation were broken. Await asynchronous tests and promise assertions so failures reach the runner. When changing a shared helper, verify that a deliberately false condition fails the suite and that an assertion-free test still triggers S2699. Do not add dummy assertions, rename tests to evade detection, or suppress the rule across a suite. Where a real producer exists, take fixtures from its output: a fixture written alongside the code can't catch a misunderstanding they share. This guidance also applies to tests written through the agent roles and skills.
