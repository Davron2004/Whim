...
function orchestrationScript(cfg) {
  return (
    ...
    // verdictSeq: parent-owned count of AUTHENTICATED probe verdicts (bumped in the kind==='probes'
    // handler above). It lives in the OUTER page, so it SURVIVES makeIframe() (iframe recreation) —
    // unlike the realm-local display "gen N", which resets to 1 on every reset and so can't tell a
