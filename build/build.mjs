...
async function main() {
  ...
  // Assemble the RN app's RUNTIME_HTML — the PRODUCT shell only: no baked app bundles, no
  // autostart, diagnostics OFF (showDiagnostics is false below: the product shell renders a
  // full-screen iframe with no on-screen probe JSON). MiniAppView always delivers the real app
  // by SOURCE (host.deliverBySource, D3) after onLoadEnd; a baked bundle here would mount and
  // PAINT before that delivery — a flash of the wrong app plus an unattributable `paint` frame.
  // The bundles this file used to embed (tip-splitter, water-counter, latency-probe,
  // pour-over-timer, and the adversarial sql-injector/cap-intruder/evil pen-test fixtures) are
  // fixture/scenario concerns, not product ones — they still exist as bundles (see APPS above)
  // and ship to the invariant suites and the launcher's app-bundles seed data via
  // runtime-artifacts.json / app-bundles.ts below, just never baked into the product HTML.
  ...
  await writeFile(
    ...
    '// #35 CSP, channel-(b) transport wired up, NO baked app bundle and NO autostart — the\n' +
    '// launcher always delivers the real app by source (deliverBySource) after onLoadEnd.\n' +
    '/* eslint-disable */\n' +
