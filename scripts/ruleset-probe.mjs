...
try {
  ...
  for (const r of active) {
    ...
    if (conditionsTargetDefault(detail.conditions) && ruleTypes.has('pull_request')) {
      const extras = ['non_fast_forward', 'deletion', 'required_status_checks'].filter((t) => ruleTypes.has(t));
  ...
  console.error(`ruleset-probe: failed to query rulesets via \`gh api\`: ${e?.message ? e.message : e}. Refuse closure.`);
  process.exit(1);
}
