 ...
 * response body. That request is counted as a bundle fetch, never as a hit. Every other HTTP
 * request is counted as a hit keyed by the variant name in its path (`/wnd/hit/<variant>` →
 * `<variant>`, anything else keyed by its raw path). A second, plain TCP listener (the "TLS port")
 * counts accepted connections and closes each socket immediately — proving the egress happened
 * without needing to terminate TLS.
 *
 * At exit (the `--seconds` timer, or SIGINT) it prints one `hit <path> <count>` line per path that
 * was hit, then one summary line:
 *
 *   NETDENY PASS|FAIL expect=<leak|zero> hits=<total hit count> tls=<tls connection count> [missing=<a,b,c>]
 *
 * `--expect zero` passes only when no path was hit and no TLS connection was accepted.
 * `--expect leak` passes only when `loc-href`, `loc-assign`, `meta-refresh`, `anchor-click` and
 * `host-top-frame` each have at least one hit AND the TLS port saw a connection; a failing `leak`
 * expectation appends `missing=<…>` naming every one of those five that didn't.
...
async function main(): Promise<number> {
  ...
  const { pass, lines } = computeVerdict(opts, hits, tlsHits);
