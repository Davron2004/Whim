...
share-vs-fresh at fork time" requirement. Scenarios 1-3 are rendered end to end through
...
3. `LauncherRoot`'s `onFork` forwards those opts verbatim into `access.fork(app, undefined,
   ...
   false` leaves it in its own group, exactly per the `storage-groups` contract.
...
4. The question lives ONLY in `HomeScreen`'s explicit-fork path. The rewind continuation — a
   rebuild of an app that is behind its tip — calls `access.fork(entry, undefined, { shareData:
   true })` directly, bypassing `HomeScreen` and never routing through this sheet
   (`build-lifecycle.suite.ts`, behind-tip rebuild).
