...
4. `./scripts/gate.sh` — run it yourself, in your worktree. You are NOT done until it prints `FAST GATE PASSED`. Fix and rerun. If it refuses with `node_modules is not its own`, STOP (class B) and report it: your worktree was not provisioned by `scripts/worktree.sh`, and only the orchestrator re-provisions it. Never create, replace or symlink `node_modules` or `node_modules/@whim/*` yourself — a symlink makes your tests run the primary tree's code.
