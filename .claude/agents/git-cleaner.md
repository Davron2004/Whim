...
Hard rules:
- Touch ONLY the branch `cleanup/<ID>-squashed`. Never any command naming `main`, the TARGET branch itself, or tags as a write target. Never `git worktree` anything.
- These are policy-denied — do not attempt, do not retry: `push`, `pull`, `fetch`, `clone`, `remote`, `config`, `reflog`, `gc`, `update-ref`, `symbolic-ref`, `tag -f`, `tag -d`, forced moves of `main`/`integration/*`. `rebase` is allowed in your lane but the snapshot rebuild above is strictly simpler — prefer it.
