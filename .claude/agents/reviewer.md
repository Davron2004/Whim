...
greenBy suites (phased TDD — docs/harness.md §6): the suite summary line reports **XPASS** — a test tagged for a later chain that already passes. Treat every XPASS on a chain you are auditing as a vacuity suspect: confirm the test genuinely exercises code that does not yet exist (or is not yet wired), not that it passes trivially. Same weight as a tautological test (step 2) — an unexplained XPASS is a finding. Conversely, when a chain claims to *promote* a test to green, verify its code is what turned it green, not a loosened assertion.
