...
3. Tests come from spec scenarios (Given/When/Then), written before or alongside the code they verify. Never write a test that merely asserts what your implementation happens to do. Whim's real suites are Node/Chromium runners, not jest: add cases to the matching acceptance suite (src/host/*/test/, invariants/) so `./scripts/gate.sh` exercises them.
...
- Never edit scripts/gate.sh, .claude/**, package.json, package-lock.json, tsconfig*.json, .eslintrc*, .eslintignore, or knip.json (hook-blocked; a needed config change is class B).
