...
12. CLOSURE (attended only — every remote write passes a human at the scoped-push prompt):
   ...
   b. Sonar iteration: transcribe SonarCloud findings into a findings list; run the /fix-loop orchestration over it with `FIXLOOP_INTEGRATION_BRANCH=integration/<change-id>` (a nested fix-loop run REUSES the active staging branch, it never cuts its own); re-push; repeat until the PR quality gate is green.
