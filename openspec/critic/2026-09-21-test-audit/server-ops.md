...
- [CHANGE-DETECTOR] profile product pins (L699-709) with their reds (L1722-1729) — DELETE (**except "standard must not override server limits", kept: overturned 2026-09-21, see verification.md**). `standard must use e2-standard-2` and `event must provide … 15/6/32` restate the profile files. The event values are pinned again by the exact `config.env`/`compose.env` expectations at L1040-1054. Keep the relational checks: allowed keys, loads, synthrun ≤ vCPUs, synthrun ≤ generation cap, unique machine types.
...
> 2026-09-21, carrying out: `InMemoryUsageStore` (Incidental) is **not** deleted; see verification.md "Overturned while carrying out the audit".
...
- [CHANGE-DETECTOR] `check('privacy.html names OpenRouter …', renderedPolicy.includes('through OpenRouter'))` (L116) — ~~DELETE~~ **REWRITE (overturned 2026-09-21, see verification.md)**. It's an exact-phrase pin: "via OpenRouter" would break it, and a substring can't enforce the disclosure anyway.
...
- [TAUTOLOGY] `check('the resolved row is the one the provider answered for', lateId !== neverId)` and the mislabeled L394 (**L394 kept and relabelled: overturned 2026-09-21, see verification.md**) ("keeps its cost", which actually asserts `unresolvedCount`) (L394-395) — DELETE.
