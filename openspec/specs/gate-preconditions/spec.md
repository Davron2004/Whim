...
Created by syncing the `harden-gate-preconditions` change. The verification harness checks its own
preconditions and postconditions before trusting a result, and fails with the *accurate* cause
rather than a misleading downstream symptom.

The motivating defect: `git checkout` reports success per invocation, not per file — when it cannot
write a tracked path it warns, updates everything it can, and still exits `0`. A half-applied tree
then reached the gate, whose tamper tripwire compared it against the pinned baseline and reported a
verification-config mismatch, so an operator had to disprove a tamper accusation to discover a
failed checkout. A verification harness that can silently half-apply is worse than one that refuses
to run: everything downstream inherits "the gate passed" as an unverified assumption.
...
#### Scenario: Baseline helper invoked in a command substitution

- **WHEN** a helper that resolves the baseline is invoked inside a command substitution, where a non-zero exit cannot terminate the calling shell
- **THEN** the caller SHALL check the helper's status explicitly, and SHALL NOT proceed with an empty baseline
...
#### Scenario: Restore on an early-exit path

- **WHEN** the command exits early, before the gate runs, and the exit handler restores the working tree
- **THEN** that restore SHALL be verified and its diagnostics preserved on the same terms as the normal path, because an early exit is when the restore is most likely to be impeded
...
#### Scenario: Fixture barrier cannot be established

- **WHEN** the suite's fixture relies on a write barrier that a privileged user would bypass
- **THEN** the suite SHALL verify the barrier actually took effect and SHALL fail loudly if it did not, rather than skipping or passing
