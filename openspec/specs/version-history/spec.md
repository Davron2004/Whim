...
than version-control vocabulary. Covers envelope-aware prompt rendering with a raw-string
fallback, instant tap-to-restore-before-that-prompt with single-tap undo, roll-forward (restored
versions stay listed and reachable), named pins, forking any listed version into its own app, and
data-shape annotations plus restore reassurance for schema-changing versions.
