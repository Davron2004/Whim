...
## Purpose
The harness server skeleton: a Node 22 Hono service exposing the SSE generation endpoint (over
a stub pipeline in this change), the canned rewrite endpoint, device-UUID identity middleware,
durable per-device token metering (the only server state), usage readback, and the
model-agnostic OpenRouter client wrapper (built but unmounted). LAN dev only — no real
generation, deployment, or TLS yet.
