...
Created by syncing the `automate-closure` change. Defines how the bash-policy hook judges shell compound commands (`&&`, `||`, `;`, `|`) instead of blanket-denying or blanket-allowing them: unroll into simple-command segments, judge each against the existing per-command policy, and take the most restrictive segment verdict — while failing closed (never silently allowing) whenever the command's composed identity can't be determined statically.
