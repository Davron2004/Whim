# generation-stream-transport Specification

## Purpose
How the device consumes the `POST /v1/generate` SSE stream. React Native's global `fetch` is the
`whatwg-fetch` polyfill over `XMLHttpRequest` and defines no `Response.body`, so this route streams
over `XMLHttpRequest`'s incremental delivery instead. Covers transport selection (decided up front
