# sdk-charts Specification

## Purpose
The `Chart` SDK export — the mini-app charting surface. Exactly one declarative component
(`kind: 'bar' | 'line' | 'heatmap'`) renders the three corpus chart kinds from plain data,
pure rendering with no bridge traffic. Colors derive from the active theme's existing roles,
so a theme switch recolors charts with no mini-app-side handling. Degenerate data (empty,
all-zero, single-point, non-finite) renders safely instead of throwing. Rendered entirely as
inline SVG/DOM under the unchanged containment contract — no canvas, no network, no new
tokens.
