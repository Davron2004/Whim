...
Created by syncing the `automate-closure` change. Defines the programmatic path for pulling SonarCloud findings for a PR into the existing fix-loop findings pipeline, replacing human transcription from the GitHub check-run UI, while keeping the server-side SonarCloud quality gate as the actual merge-blocking enforcement.
