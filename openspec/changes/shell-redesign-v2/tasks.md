...
- [x] A1 `src/sdk/theme.ts` carries the fixed v2 shell palette, status hues, radii, spacing and
...
- [x] A2 `PRESETS`, `ACCENTS`, and the shape variants are deleted; `ThemePref` no longer carries
  ...
  mechanism (design D5).
- [x] A3 `src/sdk/tokens.ts` resolves the three faces (Instrument Sans / IBM Plex Mono / Newsreader
...
- [x] A4 Export `appColor(name: string): string` from the SDK theme module — pure, deterministic,
...
- [x] A5 The seven static TTFs are present under `android/app/src/main/assets/fonts/` with canonical
...
- [x] A6 `src/host/launcher/theme.ts` + `theme-context.tsx` expose the fixed theme and drop the
  preference plumbing; `shellPalette` maps the v2 roles.
- [x] A7 `SettingsScreen.tsx` no longer renders a preset, accent, or shape picker (delta:
...
- [x] A8 `src/host/launcher/test/theme.suite.ts` is rewritten: the preset/accent iteration assertions
...
- [x] B1 Create `src/host/ui/whim-prose/` (shell-side, **not** exported from `vc-sdk` — design D7)
...
- [x] B2 Add the renderer: merges lexer spans with producer-supplied `chg`/`hedge` marks and renders
...
- [x] B3 Enforce the discipline caps **in the renderer**: ≤4 system marks per sentence, ≤1 coloured
  ...
  Syntax renderer".
- [x] B4 Wire the off-switch: with highlighting off the renderer emits no class-bearing span on any
  screen. Read the flag from the setting A7 persists; do not add a second flag.
- [x] B5 Seed `src/host/launcher/copy.ts` with every new v2 string this change needs, taken verbatim
...
- [x] B6 Add `whim-prose.suite.ts` under `src/host/launcher/test/` (registered in
...
- [x] C1 `contract/src/index.ts`: add `ClarifyRequest`/`ClarifyResponse` (≤3 questions, each with
  non-empty options; an empty list is valid). Delta: `generation-contract` §"Clarify request and
  response shapes".
- [x] C2 `contract/src/index.ts`: add optional `clarifications` to `GenerateRequest` and
...
- [x] C3 `contract/src/index.ts`: add optional `summary { text, kind, touched, marks }` to the
...
- [x] C4 `src/sdk/index.tsx`: add optional literal `tileColor` to `AppSpec` (statically extractable,
  never introspected). Delta: `sdk-design-system` §"An app declares its one tile colour".
- [x] C5 Server check/machine stages: extract `tileColor` into `CheckedManifest` in the **same single
...
- [x] C6 Add `POST /v1/clarify` to the server route table, behind the existing device-identity
...
- [x] C7 Apply the device gate by **path prefix** over `/v1` rather than route by route, so a route
  added later is gated by construction. Delta: `generation-server` §"Device-identity middleware".
- [x] C8 Thread `clarifications` into the rewrite call and return `plan` rows when the model produces
  them. Delta: `generation-server` §"Rewrite endpoint over the real rewrite model".
- [x] C9 Add the post-run summariser step to the pipeline: one plain-words sentence + `kind` +
...
- [x] C10 Assert in the server suite that the SSE route frames the summary through unmodified —
  neither synthesized nor stripped. Delta: `generation-server` §"SSE generation endpoint…".
- [x] C11 Extend the server acceptance suite (existing entry point, no new npm script) with: clarify
...
- [x] D1 Restructure the `Screen` union and the render chain in `LauncherRoot.tsx` to the five-step
...
- [x] D2 Build the compose step: headline `What should it do?`, the prompt field (never live-lexed),
  the helper line, the `Or start from` chips (fill only, never advance), and the bottom primary
  action. Delta: same requirement.
- [x] D3 Call `POST /v1/clarify` between compose and plan; render ≤3 questions as single-select
...
- [x] D4 Build the plan step: `Here's the plan`, the labelled rows (or the single-row fallback when
...
- [x] D5 Build the build step: the four named steps derived from `stage` events
...
- [x] D6 Build the done step: the app's own tile, `<App name> is ready`, the body line, and the two
...
- [x] D7 Every busy primary action reads `One moment`; `Continue` on compose/clarify and `Build it`
  on plan; no bare-spinner-only control anywhere in the flow.
- [x] D8 Bump the prompt envelope to `{v:2, text, summary?}` in `prompt-envelope.ts` and at the
...
- [x] D9 Rebuild `HomeScreen.tsx` to the `2a` home: `Whim` title, `Your apps` eyebrow, the 3-column
...
- [x] D10 Add grid and plan-row skeletons whose geometry comes from the components' exported size
...
- [x] D11 Update `prompt-flow-screens.suite.ts` and `prompt-flow-wiring.suite.ts` to the new machine:
...
- [x] E1 Rebuild `HistoryScreen.tsx` to the `4a` timeline: header naming the app in its own hue over
...
- [x] E2 Feed rows from the stored summary, falling back to the version's prompt text when there is
  none (older envelope, raw string, or a run that produced no summary). Delta: same requirement.
- [x] E3 Mark the active version `↑ you're on this one`, derived live from the store, never persisted
  on the app record.
- [x] E4 Make a tap **expand** a row rather than restore, one row open at a time. Delta:
  `version-history` §"Tapping a row expands it; restoring is confirmed, never instant".
- [x] E5 Build the expanded body: the result sentence, the `What it touched` chips (area names, never
  ...
  restore reassurance".
- [x] E6 Build the filter pills: the all-versions pill with a live count plus one pill per summariser
...
- [x] E7 Build the confirm sheet for restore and for copy: safe option as the large button,
...
- [x] E8 Remove the pin surface from the history screen (labels on rows, the pin action). Do **not**
  remove the store verbs or the `StoreAccess` wrappers; stored pins stay stored. Delta:
  `version-history` REMOVED §"Named pins".
- [x] E9 Update `history-logic.ts` for the new row model (summary-or-prompt headline resolution, kind
...
- [x] E10 Screen `4b` is not built. Do not add a "last change, undoable in place" surface.

## F. Tile colour plumbing + the 2b tile

- [x] F1 Extend the host-held `AppManifest` (`src/host/bridge/contract.ts`) with the optional
...
- [x] F2 Add the resolution helper: declared colour when present and valid, `appColor(name)`
...
- [x] F3 Reduce `src/host/launcher/tiles.ts` to `monogram` plus a delegation to `appColor` — delete
  `TILE_COLORS` so no second name→hue mapping survives (design D6).
- [x] F4 Build the `2b` ghost-letterform `AppTile` component: square, tile radius, solid app colour,
...
- [x] F5 Add the record-mapping module that lifts the wire manifest's colour onto the host record, so
  the mapping is testable without mounting the shell. Group D consumes it; it does not itself edit
  `LauncherRoot.tsx`.
- [x] F6 Add a suite (registered in `test/acceptance.ts`) covering: declared colour wins; malformed
...
- [x] G1 Replace `FloatingExit.tsx` with the orb: ink circle, tap opens the menu, tapping the orb
...
- [x] G2 Populate the menu with cheap, undoable actions only — no delete, rename, or restore. The
  versions action opens the versions sheet (screen `4b` does not exist).
- [x] G3 Persist a per-action tap count through the launcher's existing key-value path. No counter or
  count appears on any user-facing surface.
- [x] G4 Ship **no** wheel affordance: no press-and-hold arming, no directional flick, no wedge
  geometry, and no copy advertising one — no per-row direction hints and no "hold to flick" caption.
- [x] G5 Add a suite (registered in `test/acceptance.ts`): open/close/dismiss transitions fire no
...
- [x] H1 Append decisions to `docs/decisions.md` recording: the three answered OPEN questions
...
- [x] H2 Update the `sdk-design-system`, `app-launcher`, `prompt-flow`, `version-history`,
