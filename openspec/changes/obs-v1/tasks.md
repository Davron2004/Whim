...
- [x] A1 `package.json`: add device dependencies `react-error-boundary@6.1.2` and
  `react-native-logs@5.6.0`; add the script `whim:logs` pointing at `server/logs-tail.mjs`, the
  tail utility group **F** creates (F5). `package-lock.json` regenerated by `npm install` (not
  hand-edited).
- [x] A2 `server/package.json`: add runtime dependency `pino@10.3.1` and **dev** dependency
...
- [x] A3 `.eslintrc.js`: add two further `no-restricted-syntax` entries matching **discard-shaped**
  catches only — `CatchClause[body.body.length=0]`, and a bindingless
  `CatchClause[param=null][body.body.length>0]` containing neither a `ThrowStatement` nor a call
  into the logging seam (a member call on the seam logger `log`, or a `log`-prefixed helper) —
  each with a `message` naming the ways to satisfy it (rethrow, log through `src/host/logging/`,
  or a documented disable). A catch that binds and handles the error is deliberately outside the
  selectors (the as-designed broad selector measured 108 fires on legitimate handlers; the
  discard-shaped pair measures 62, all real swallows). Core rule only: no new plugin, no new
  dependency, legacy config (design D9). Delta: `host-observability` §"No error is swallowed
  silently".
- [x] A4 `knip.json`: add `pino-pretty` to `ignoreDependencies` (reached as a transport target, not
...
- [x] A5 Comment every empirical tripwire fire (design D8; 62 sites measured 2026-08-08, list in
  `progress.md`), two species:
  - **22 interim markers** — `// eslint-disable-next-line no-restricted-syntax -- obs-v1-interim:
    <reason>` — on the silent catches in this change's jurisdiction (`src/host/launcher/**`,
    `src/host/bridge/**`, `src/host/cue-backend.ts`). The token `obs-v1-interim` is fixed and
    greppable; group E resolves every one of them.
  - **40 permanent documented disables** — `-- intentional: <site-specific reason>` — on the
    deliberate best-effort catches in subsystems this change does not touch (version store,
    storage engine, SDK, synthrun, server internals, repo tooling), as the delta spec sanctions.
  The originally-listed `synthrun/test/acceptance.ts:202,665` are **not** sites: they sit inside
  template-literal fixtures the acceptance test compiles, invisible to lint, and a marker there
  would corrupt the fixture. `LauncherRoot.tsx`'s alert catches bind and display the error, so
  they are not discard-shaped and take no marker; E3 still adds their seam records.
- [x] A6 Verify the batch: `npm install`, then `npm run lint` (green — the tripwire fires nowhere),
...
- [x] B1 Create `src/host/logging/` with the channel registry: the declared channel constants
...
- [x] B2 Build the logger on `react-native-logs@5.6.0`: ordered severity levels, a per-channel
...
- [x] B3 Add the redaction step: any field whose name is in the sensitive set (prompt text,
...
- [x] B4 Add the ring-buffer transport: fixed capacity, oldest-evicted, never persisted, exposing a
...
- [x] B5 Add the batching HTTP sink transport: bounded by record count **and** flush interval,
...
- [x] B6 Add `contract/src/dev-log.ts`: the record and batch **types** and the sink route's path,
...
- [x] B7 Add `src/host/logging/test/logging.suite.ts`, registered in
...
- [ ] E6 Resolve every remaining `obs-v1-interim` site (the full 22-site list is in A5's applied
  ...
  `src/host/bridge/launch.ts:47` — migrates to log through the seam or rethrow. Test-harness
  sites — `deliver-by-source.desktop.mjs:70,83`, `orb-menu.suite.ts:94`,
  `prompt-flow-wiring.suite.ts:302,315`, `theme.suite.ts:106` — migrate, or convert to a
