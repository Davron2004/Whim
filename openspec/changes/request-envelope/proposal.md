...
- **A minimum supported build per platform** (`WHIM_MIN_BUILD_IOS`, `WHIM_MIN_BUILD_ANDROID`, default `0` = off). Below it, every `/v1` route answers the new refusal code `update_required`. `/healthz` also reports the minimums, so the app can check at launch, usually before anyone types a prompt.
