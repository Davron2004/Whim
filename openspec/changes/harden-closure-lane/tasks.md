...
- [ ] 3.1 State in `docs/harness.md` §4's bash-policy row that the two relaxed forms are relaxed **only as bare, non-compound commands** — `bash-policy.sh:170` drops any command containing `&`, `;`, `|`, backticks, `$(`, `>`, `<` or a newline back to the tier-1 deny, and the denial message names the network rule rather than the compounding, which is a misleading diagnosis
- [ ] 3.2 Fix `apply.md` step 12g, which currently writes the two relaxed forms joined ("`git fetch origin` + `git pull --ff-only origin main`"): instruct them as two separate bare commands and say why. This is the exact trap that produced `research.md` §F2's wrong conclusion
