...
- [x] 1.3 Verify the SonarCloud project key `Davron2004_Whim` is visible with the token via `api/components/show` (404 anonymously is expected — the project is private) *(VERIFIED: `node scripts/sonar-pr-issues.mjs --verify` → "VISIBLE: … Davron2004_Whim (components/show 200)", run unsandboxed to stand in for the not-yet-active carve-out)*
...
- [x] 7.1 Run `scripts/gate-full.sh` green on the change tip (includes the new unroller, policy, and ingestion suites) *(**DEFINITIVE PASS 2026-07-26** at tip `1f38341`, from the clean primary tree:
  `FIXLOOP_INTEGRATION_BRANCH=integration/automate-closure ./scripts/fixloop.sh gatefull integration/automate-closure`
  → `FULL GATE PASSED` — fast gate incl. all new suites [unroller 46, bash-policy 44, sonar 10],
  knip, `guard:metro`, all three Chromium invariant suites, codex-sync, `openspec validate` 28/28.
  Two prerequisites, both non-obvious — see the gatefull divergences in `dispositions-sonar-1.md`:
