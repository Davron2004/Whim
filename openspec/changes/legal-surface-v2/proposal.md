...
- **Docs and attended work:** a breach runbook and breach log, drafts of the Quebec s.17 assessment and the UK Children's Code DPIA, and a post-launch lawyer brief (decision 7: no lawyer before launch). The owner does the store-console steps, the OpenRouter account setting, the mailbox, the identity values, the EU/UK representatives, the Texas §121.053 store notice, the fluent French and Korean checks, and the on-device network-deny runs (B7).
