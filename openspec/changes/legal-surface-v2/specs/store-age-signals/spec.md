...
Before showing the terms step, the launcher SHALL ask the platform for the store's age signal (Apple Declared Age Range on iOS, Play Age Signals on Android) when no age-check outcome is stored, when the stored one is older than 30 days, or when the stored one is blocked. It SHALL reduce the answer to one of `adult`, `minor-approved`, `minor-not-approved` or `unavailable`. `minor-not-approved` SHALL keep the AI features off and show a message saying a parent can approve Whim through the store. Every other result SHALL let the flow continue. Unsupported OS versions, regions without a signal, and API errors SHALL count as `unavailable`.
