...
No chain is HUMAN-BOOTSTRAP. None edits `scripts/gate*.sh`, `scripts/fixloop.sh`, `.claude/**`, `.codex/**`, `invariants/`, `build/*`, `package.json`, `package-lock.json`, `tsconfig*.json`, ESLint config, `knip.json`, `babel.config.js`, `metro.config.js` or `src/host/launcher/test/run.mjs`. No dependency is added. Every new export has a consumer (the hook and table in `LauncherRoot.tsx` and the screens; `bindSystemBack` in the hook; `planBackAction`/`consentControls` in their screens), so knip passes without touching `knip.json`.
