...
→ daily units → content policy. Every refusal after `acquire` succeeded MUST `release()` first; a
THROW past it must ALSO `settle(requestId,{outcome:'error'})` its row (no refund) before releasing —
`app.ts`'s one `onError` answers `500 {error:'internal_error'}`, logging the real message + path.

## `server/src/admission/slots.ts`

```ts
export type SlotKind = 'generate' | 'unary' | 'probe';
export const DEFAULT_MAX_CONCURRENT_PROBES = 2; // /healthz/sse's own pool, never the unary pool
...
export interface SlotLimits { maxConcurrentGenerations: number; maxConcurrentUnary: number; maxConcurrentProbes?: number }
...
- One per process, from `ServerConfig.maxConcurrentGenerations`/`maxConcurrentUnary`/`maxConcurrentProbes`
  (`WHIM_LIMIT_PROBE_CONCURRENCY`, default 2 — every cap is env-read).
- `acquire` order: `draining` (all kinds) → `device_busy` (generate only; unary/probe have no
  per-device exclusivity) → `at_capacity` (its own cap per kind, `maxConcurrentProbes` for probe).
- `release()` is idempotent per handle: the first call frees the slot, every later is a no-op — also
  after `startDraining()`; a stale handle never frees the same device's newer generation. Safe from
  an abort listener AND a `finally` AND a post-acquire refusal.
- `startDraining()` is one-way; the report route checks `isDraining()` without acquiring. O(1).
- `/healthz/sse` always `acquire('probe','healthz-probe')` — own pool, never `unary`; drain waits on it.
...
hint: 'Include a UUID in the x-whim-device request header.' }`; not an 8-4-4-4-12 hex UUID (any case)
→ `400 { error: 'invalid_device_id', hint: 'The x-whim-device header must be a valid UUID (e.g.
xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx).' }`; else `{ ok: true, deviceId: <header> }`.
...
- Failure → `{ ok: true, lookupFailed }`; the caller MUST log a warning when it is set. Malformed =
  unparseable JSON, no `data`, missing `limit_remaining`, or a non-number non-null value. A non-2xx is
  never parsed. A failure caches nothing and never falls back to a stale value.
