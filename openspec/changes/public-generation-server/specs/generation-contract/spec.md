...
The contract SHALL define `ServiceRefusalCode` as the closed set `payload_too_large | daily_limit | device_busy | server_busy | content_policy | policy_unavailable`: the `error` identifiers a conforming server uses for size, admission, and content-policy refusals.
...
- **THEN** parsing fails, and each of the six listed identifiers validates
