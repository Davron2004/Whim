...
- **One request id per `/v1` request.** Every log line for a request carries it, the ledger row records it along with a new closed-set `failure_reason` column, and the device gets it in an `x-whim-request-id` response header and sends it back with any diagnostic about that request.
...
- `server-observability`: log shipping to Cloud Logging with severity, the per-request id (log lines, ledger row, response header), the ledger's `failure_reason`, and the alerting set (uptime, budget, reports, failure spikes, refusals, diagnostics).
...
- **Contract:** the `DiagnosticsBatch` zod schema and the `x-whim-request-id` header name in `@whim/contract`. The device imports them type-only.
- **Server:** new `routes/diagnostics.ts`; request-id middleware in `app.ts`; `usage-store.ts` (additive `failure_reason` column via the existing `ALTER TABLE` pattern); `machine.ts` terminal failure; pino `severity` formatter in `logger.ts`. No new runtime dependency.
