...
- [x] 1.1 `copy.ts`: change `failureDismiss` to 'Discard this attempt'; add
      `failureBack: 'Back to your apps'`.
- [x] 1.2 `FailureScreen.tsx`: add an `onBack` prop; render Back (`failureBack`) as the plain
...
- [x] 1.3 `LauncherRoot.tsx`: wire `onBack` to the same navigate-home transition the dismiss path
      ...
      and live-failure).
- [x] 1.4 Hardware back on the failure screen performs the non-destructive Back (rewire if it
      currently dismisses; assert either way).
- [x] 1.5 Suites: update `failure-screen.suite.ts` copy/source locks; add behavioural coverage in
...
- [x] 2.1 `npm run launcher:test` and `npm run lint` green; `./scripts/gate.sh` PASS.
