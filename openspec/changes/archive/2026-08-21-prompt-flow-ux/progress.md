...
## 2026-08-21 — On-device acceptance (task 7.2)

Run attended on the Pixel 9 Pro XL emulator (release APK, Maestro-driven), stub server first, then the real pipeline. Verified: Settings server address (silent save, no connectivity check — noted), full compose→clarify→plan→build→done against the real pipeline with a rendering, functional delivered app; re-prompt at tip updates the same tile (no duplicate) and renders; fork-from-old-version via History produces the second tile (confirm sheet per the post-redesign version-history spec — the "silently" wording in the task text predates the redesign); device id persists across force-stop (same UUID in MMKV + continued server identity). Deviations: the `[[fail]]` hook is unreachable end-to-end because the rewrite stage paraphrases the literal marker away — filed as finding F5 in openspec/findings-blank-screen-2026-08-21.md (fix-loop), to be re-verified in stub mode after it lands; the honest-failure UI itself WAS demonstrated live via the interrupted-build ghost tile ("Couldn't build this app"). Stub-mode blank screens observed during the first pass were root-caused to the stub bundle defining no AppSpec (finding F4), not a prompt-flow defect. Ticked 7.2.
