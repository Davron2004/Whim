...
- [x] 1.1 Add `containment_unobserved` to `DIAGNOSTIC_KINDS` in `checks/contract.ts`, in the
...
- [x] 1.2 Add `containment_unobserved` to `RUNTIME_OBSERVED_KINDS` in `synthrun/observe.ts` and give
...
- [x] 1.3 Update the kind-roster assertion in `checks/test/acceptance.ts` so the closed union and the
...
- [x] 1.4 Write `handoff/diagnostic-kind.md`: the exact kind string, its severity, its hint text, the
...
- [x] 2.1 Move relay installation out of `EarlyObservers.finish` into the pre-navigation phase
...
- [x] 2.2 Keep the installation **main-frame-confined**. Do not use `context.addInitScript` or any
...
- [x] 2.3 Move the console listener into the same pre-navigation phase, so `rnLog()`'s console
  fallback and `{__whimHostLog:true,line}` frames emitted before `load` are not lost either.
- [x] 2.4 Settle the `waitUntil` mode explicitly (design Open Question 2). Do not change it
...
- [x] 2.5 Update both compositions for the new phase split: `openObservedRun` (`observe.ts`) and the
  production composition in `report.ts`, so `finish(ctx)` retains only what genuinely cannot run
  before navigation.
- [x] 2.6 Record in `progress.md` any shift in what `bootMs` / `mountToPaintMs` measure. The
  `synthetic-run` spec's determinism requirement is "timings aside", so a shift is permitted — a
  silent one is not.
- [x] 2.7 Write `handoff/observation-phases.md`: the post-move signature and responsibilities of
...
- [x] 3.1 Widen `RunReport.contained` from `boolean` to `boolean | null` in `synthrun/contract.ts`
...
- [x] 3.2 In `report.ts`, emit `obs.state.contained` verbatim instead of `obs.state.contained === true`.
  Leave the separate `paintAtMs ?? budgets.mountBudgetMs` collapse alone — it is a different field
  and a different question.
- [x] 3.3 In `recordProbesOutcome`, keep `containment_failure` on an explicit `false` only, and emit
...
- [x] 3.4 Record rejected forgeries as the **fact** plus a **bounded** count on the report, capped at
...
- [x] 3.5 Write `handoff/run-report-contract.md`: the widened `contained` type verbatim, the forgery
...
- [x] 4.1 Un-quarantine the `mount_timeout` case in `synthrun/test/acceptance.ts` and make it sound:
...
- [x] 4.2 Add the D7-local assertion: from inside the candidate's opaque-origin sandboxed realm, the
...
- [x] 4.3 Add cases separating the three verdict states end-to-end: a contained run (`true`), a
...
- [x] 4.4 Keep the forged-verdict cases green (the existing forged-verdict case and the six-way
...
- [x] 4.5 Triage suites that move because frames previously dropped in the race now arrive. A broken
  assertion here is a finding to be understood and recorded, not noise to be re-baselined.
- [x] 4.6 Run `npm run synthrun:test` (gate-full only) green, and confirm no quarantined case remains
  in the file for this cause.

## 5. Pipeline consumers of the widened type (D3, D6)

- [x] 5.1 `server/src/generation/stages/run.ts` — discriminate all three values explicitly rather than
...
- [x] 5.2 `server/src/generation/machine.ts` — model the unobserved state as a **third arm of the
...
- [x] 5.3 Add a distinct user-facing reason constant beside `CONTAINMENT_FAILURE_REASON`, with the
...
- [x] 5.4 Update `server/test/e2e.ts`: `fakeReport`, `containedDetail`, and the fixtures around
...
- [x] 5.5 Assert no forgery or unobserved-verdict detail reaches any assembled prompt (the
  prompt-injection guard in the `generation-pipeline` delta).

## 6. Evals consumers

- [x] 6.1 `evals/adapters/synthetic-run.ts` — stop hardcoding `containment.authenticated: true`
...
- [x] 6.2 Update `evals/test/fixtures/synthetic-run-report.json` and any adapter/tier test fixture the
  widened type touches, so the three states are represented rather than only the passing one.
- [x] 6.3 Add an adapter test pinning the mapping: `true` → authenticated+contained, `false` →
