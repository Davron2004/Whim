...
- [x] 1.1 Write the English test spec for the deterministic suite before any implementation:
...
- [x] 2.1 Add `"workspaces": ["contract", "server"]` to root `package.json`; scaffold
      ...
      and `.env`.
- [x] 2.2 Add `npm run guard:metro` (release-mode `react-native bundle` of `index.js` to a
...
- [x] 2.3 Verify the existing gates still pass untouched on the new layout: `npm run build`,
      `npm run invariants`, `npm run lint`.

## 3. Contract package (TDD)

- [x] 3.1 Implement `@whim/contract` schemas per design D3: `GenerateRequest`,
...
- [x] 3.2 Implement the contract tests from 1.1 (round-trip, rejection, dependency-budget)
      in the acceptance suite skeleton (`server/test/run.mjs` esbuild-bundle-then-run idiom).

## 4. Server core (TDD)

- [x] 4.1 Hono app factory (`@hono/node-server` only in `main.ts`/`dev.mjs`, app itself
      framework-portable): `GET /healthz`; `x-whim-device` UUID middleware on `/v1/*` with
      structured `400`s; tests via in-process `app.request()`.
- [x] 4.2 `npm run server:dev` (`server/dev.mjs`, esbuild-bundle-then-run; `0.0.0.0`,
      `WHIM_SERVER_PORT` default 8787).
- [x] 4.3 SSE writer: `event:`/`data:`/monotonic `id:` framing + injectable keepalive
      interval; test-side SSE reader for the suite.

## 5. Stub pipeline + endpoints (TDD)

- [x] 5.1 `Pipeline` interface + `stubPipeline` per design D5: canned plan→generate(`token`s)
      ...
      0 in tests).
- [x] 5.2 `POST /v1/generate`: body validation (`400` JSON, never SSE, on invalid), stream
      the pipeline through the SSE writer, exactly-one-terminal-then-close.
- [x] 5.3 `POST /v1/rewrite`: validated canned deterministic rewrite.

## 6. Metering (TDD)

- [x] 6.1 `UsageStore` interface + `node:sqlite` implementation (one table under
      `WHIM_DATA_DIR`, `:memory:` for tests); restart-durability test; nothing-but-the-counter
      data-dir inspection test.
- [x] 6.2 Credit stub-run usage through the store; `GET /v1/usage` readback scoped to the
      calling ID, zeros for unknown IDs.

## 7. OpenRouter wrapper (TDD, unmounted)

- [x] 7.1 `openrouter.ts` per design D6: model-id-as-parameter, async-iterable text deltas,
