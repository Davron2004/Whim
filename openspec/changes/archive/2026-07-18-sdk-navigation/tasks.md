...
## 6. Reviewer remediation

- [x] 6.1 Make screen-graph navigation resolution binding-aware: recognize direct, aliased, and namespace `vc-sdk` nav imports; ignore unrelated/local/shadowed `nav`; add red/green acceptance for literal and non-literal targets in every spelling; add `navigation-demo.app.tsx` to the honest fixture corpus
- [x] 6.2 Extract the nav emitter, public `nav`, `NavRoot`, and private machinery into `src/sdk/navigation.tsx`; make SDK acceptance import the root directly from that internal source module; temporarily retain barrel re-exports so the current loader stays gateable; update `handoff/nav-api.md`
- [x] 6.3 HUMAN-BOOTSTRAP an atomic loader-only root closure: inject `NavRoot` through a transient frozen internal bootstrap global, capture and delete it before bundle delivery, fail closed on invalid/missing/undeletable bootstrap state, remove `NavRoot`/`NavRootProps` from the public `vc-sdk` barrel, assert public/runtime invisibility, and rerun build, SDK acceptance, containment/bridge invariants, production Chromium navigation, and the fast gate
- [x] 6.4 After remediation full-gate and reviewer pass, append the decision-log verification correction recording desktop Chromium acceptance and leaving Android System WebView acceptance pending
- [x] 6.5 Make shared lexical binding resolution declaration-order independent by predeclaring each scope's bindings; prove later-declared/hoisted direct, aliased, and namespace shadows are ignored while real SDK navigation remains checked
- [x] 6.6 Add durable negative regressions for public `NavRootProps` type invisibility and built-runtime bootstrap deletion plus missing/invalid/undeletable fail-closed behavior using the existing SDK and desktop-delivery verification lanes
