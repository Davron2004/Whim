...
- [x] 5.2 On-device acceptance (attended, human-run): fork an app, add versions on the original, open History on the fork → only the fork's own line; roll the original back past the fork point → the fork's versions are not listed/restorable; record `timeline` latency against #39's numbers (design D5)
