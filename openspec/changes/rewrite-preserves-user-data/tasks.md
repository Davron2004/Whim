...
- [x] 4.4 Add a pure `buildRewriteAppContext(editing)` to `src/host/launcher/generation-request.ts` (type-only contract import) reading the user-visible entry name and display names from the entry's schema artifact — test: `src/host/launcher/test/generation-request.suite.ts` (`npm run launcher:test`); gate: `./scripts/gate.sh`
