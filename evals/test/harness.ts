/**
 * Tiny assertion harness for the corpus-eval acceptance suite (task 1.2). Mirrors
 * `server/test/harness.ts` verbatim: no test framework (repo idiom), tallies pass/fail,
 * `report()` exits non-zero on any failure so `evals/test/run.mjs` gates CI.
 */
...
/** Order-independent structural equality — so a JSON round-trip whose keys come back in a
 *  different order still equals a literal written in schema order. */
